...
function Product({ product }: { product: IProduct }) {
  ...
  const updateCartInLocalStorage = (
    ...
    const cart: Record<
      string,
      IProduct & { quantity: number; cartPrice: number }
    > = JSON.parse(localStorage.getItem("cart") || "{}");

    if (cart[slug]) {
      
      cart[slug].quantity = newQuantity;
      cart[slug].cartPrice = newPrice;
    } else {
      cart[slug] = { ...product, quantity: newQuantity, cartPrice: newPrice };
    }
  ...
  const handleIncrement = () => {
    ...
    updateCartInLocalStorage(product.slug || "", newQuantity, newPrice);
  };
  ...
  const handleAddToCart = () => {
    updateCartInLocalStorage(product.slug || "", quantity, cartPrice);
  };

  // Handle notification
  const handleNotification = () => {
    toast.success("Item was add in cart sucessfully", {
      position: "top-center",
      autoClose: 2000,
